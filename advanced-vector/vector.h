...
public:
    ...
    RawMemory(RawMemory&& other) noexcept 
        : buffer_(std::move(other.GetAddress())), capacity_(other.capacity_)
    { 
...
public:
    ...
    void Reserve(size_t new_capacity) {
        ...
        MoveOrCopy(data_, new_data, size_);
        std::destroy_n(data_.GetAddress(), size_);
        data_.Swap(new_data);
    }
    ...
    iterator Emplace(const_iterator pos, Args&&... args) {
        ...
        if (Capacity() == size_) {
            ...
            if (pos != end()) {
                ...
                *(begin() + offset) = std::forward<T>(new_s);
            }
            else {
...
private:
    ...
    void MoveOrCopy(RawMemory<T>& from, RawMemory<T>& to, size_t count) {
        if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
            std::uninitialized_move_n(from.GetAddress(), count, to.GetAddress());
        }
        else {
            std::uninitialized_copy_n(from.GetAddress(), count, to.GetAddress());
        }
    }
};